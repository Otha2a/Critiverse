...
<main style="display:flex;justify-content:center;align-items:center;min-height:62vh;padding:40px 20px;">
    <div class="auth-box">
        <h2>Se connecter</h2>
        <p class="subtitle">Bon retour sur Critiverse !</p>

        <?php if (!empty($error)): ?>
            <div class="auth-error" style="margin-bottom:24px;"><?= htmlspecialchars($error) ?></div>
        <?php endif; ?>

        <form method="POST" action="/Critiverse/public/login" class="auth-form">
        ...
        <p class="auth-link">
            Pas encore de compte ? <a href="/Critiverse/public/register">S'inscrire</a>
        </p>
    </div>
</main>
